(function(){
	...
	KC3StrategyTabs.eventstats.definition = {
		...
		exportEventStats(selectedWorld) {
			/**
			 * eventSorties: {Number} represneting number of event world sorties done of this event world
			 * totalRsc: {Object} containing arrays of [fuel, ammo, steel, bauxite, buckets], keys are mapNum + init/final, e.g. 2init or 5final
			 ...
			 */
			let eventSorties = 0;
			const maps = this.maps, allPromises = [], totalRsc = {}, sortieNum = {}, shipDamages = {}, abyssalDamages = {}, taihaMagnets = {}, shipKills = {}, shipDrops = {}, 
				bossKill = {}, hqid = PlayerManager.hq.id;
			...
			for (const key in maps) {
				...
				if (mapId) {
					...
					if (this.exportFull) {
						allPromises.push(KC3Database.con.sortie.where("id").aboveOrEqual(firstSortie).and(sortie => sortie.id <= lastSortie && sortie.world == world
							&& sortie.mapnum == mapNum && sortie.hq === hqid)
							.each(sortie => {
								eventSorties++;
								if (sortie.id <= (clearSortie || lastSortie)) sortieNum[mapNum] = (sortieNum[mapNum] || 0) + 1;
								KC3Database.con.battle.where("sortie_id").equals(sortie.id).each(battle => {
									// Assign drops
									if (battle.drop > 0) {
